...
def create_parser(name, root, split='train', **kwargs):
    """
    There are 3 types of parser: 
        - ParserTfds: for tensorflow dataset (include tfds in name)
        - ParserImageTar: for tar file (include .tar)
        - ParserImageFolder: for image folder.
    """
    name = name.lower()
    name = name.split('/', 2)
    prefix = ''
